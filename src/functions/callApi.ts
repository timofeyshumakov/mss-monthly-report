...
async function executeBatch(commands: BatchCommand[]): Promise<BatchOperationResult[]> {
    const results: BatchOperationResult[] = [];

    for (let offset = 0; offset < commands.length; offset += BATCH_SIZE) {
        const chunk = commands.slice(offset, offset + BATCH_SIZE);
        const batchCommands: Record<string, BatchCommand> = {};

        chunk.forEach((command, index) => {
            batchCommands[`cmd${index}`] = command;
        });
            ...
            BX24.callBatch(batchCommands, (res: any) => {
                for (let i = 0; i < chunk.length; i++) {
                    ...
                    if (res[key] && !res[key].error()) {
                        ...
                        const error = res[key]?.error();
                        console.error(`Ошибка в batch команде ${key} (index ${globalIndex}):`, error);
                        results.push({
                            index: globalIndex,
                            success: false,
                            error,
...
export async function callApi(method: string, filter: {}, select: string[] | null, entityTypeId: number | number[] | null, batchNumber: number = 0, parsed: number): Promise<any[]> {
    let total: number = 0;
    ...
    if ((total > maxTotal && !exceptions.includes(method)) || Array.isArray(entityTypeId)) {
        let cmd: Record<string, BatchCommand> = {};
        ...
        if(iterations === 0){
          iterations = Array.isArray(entityTypeId) ? entityTypeId.length : 0;
        }
        ...
        for (let i: number = 0; i < iterations; i++) {
            ...
            if (method === "task.elapseditem.getlist") {
                ...
                if(Array.isArray(entityTypeId) && entityTypeId.length > 0){
...
export async function updateCrmItems(
    entityTypeId: number,
    items: Array<{ id: number; fields: Record<string, any> }>
...
export async function deleteCrmItems(
    entityTypeId: number,
    ids: number[]
): Promise<BatchOperationResult[]> {
...
export async function updateListElements(
    iblockId: number,
    items: Array<{ elementId: number; fields: Record<string, any> }>,
    iblockTypeId: string = 'lists'
): Promise<BatchOperationResult[]> {
    const commands = items.map((item) => ({
...
export async function deleteListElements(
    iblockId: number,
    elementIds: number[],
    iblockTypeId: string = 'lists'
): Promise<BatchOperationResult[]> {
